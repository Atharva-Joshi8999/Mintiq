...
contract Factory {
    ...
    function create(string memory _name, string memory _symbol, uint256 totalSupply) external payable {
        ...
        Token token = new Token(msg.sender, _name, _symbol, totalSupply); // deploy token
    ...
    function buy(address _token, uint256 amt) external payable {
        ...
        require(amt <= 10000 ether, "Amount exeed");    
        ...
        bool success = Token(_token).transfer(msg.sender, amt); // send tokens
